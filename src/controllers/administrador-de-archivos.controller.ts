...
export class AdministradorDeArchivosController {
  ...
  async CargarArchivoFoto(
    @inject(RestBindings.Http.RESPONSE) response: Response,
    @requestBody.file() request: Request,
  ): Promise<object | false> {
    const filePath = path.join(__dirname, ConfiguracionGeneral.carpetaArchivosUsuarios);
    let res = await this.StoreFileToPath(
      filePath,
      ConfiguracionGeneral.campoDeUsuario,
      request,
      response,
  ...
  private ObtenerArchivosPorTipo(tipo: number) {
    ...
    switch (tipo) {
      // amusement
      case 1:
        filePath = path.join(__dirname, ConfiguracionGeneral.carpetaArchivosUsuarios);
        break;
      case 2:
        break;
      case 3:
